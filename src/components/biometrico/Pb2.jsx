...
import { User , JWT } from "@digitalpersona/core"
...
class Pb2 extends Component {
  ...
  fn_startCapture = () => {
    ...
    /* reader.startAcquisition(this.componentDidCatch, InfoFingerprintReader) */
      .then(response => {
  ...
  fn_CaptureImage = () => {
    ...
    if (ListaSamplesFingerPrints && ListaSamplesFingerPrints.samples.length > 0) {
      ...
      /* let base = ListaSamplesFingerPrints.samples[0].Data;
  ...
  fn_Enroll = () => {
    const { ListaSamplesFingerPrints , reader , enroll } = this.state;
    const { ListaFingerPrint } = this.state;
    /* const find = reader.onQualityReported();
    console.log(JSON.stringify(find)) */
    const finger = enroll.CreateUser('felipe');
    console.log(JSON.stringify(finger))
    /* if (ListaSamplesFingerPrints && ListaSamplesFingerPrints.samples.length > 0) {
      let base64 = ListaSamplesFingerPrints.samples[0];
      console.log(base64)
    ...
    return (
      <div>
        ...
        <script src='/modules/WebSdk/index.js' defer></script>
          ...
          <button onClick={this.fn_Enroll}>Enrollar</button>
